...
if (!SECRET_TOKEN) {
    console.error('❌ FATAL ERROR: SECRET_TOKEN environment variable is not set!');
    console.error('Please create a .env file with SECRET_TOKEN defined.');
    console.error('You can generate a secure token with: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'hex\'))"');
    process.exit(1);
}
...
// Session diagnostics - SECURED (moved after auth middleware will be applied below)
...
// Session diagnostics (SECURED - requires authentication)
...
// Logger - Use pino-pretty only in development
const logger = pino({
